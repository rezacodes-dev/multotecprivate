	...
	if( $cms_link_id != '' && $cms_link_id != null ) {
		...
		if( isset($cms) && !empty($cms) ) {
			...
			if( $tabType == 'PRODUCT_CATEGORY' ) {
				$data = DB::table('product_categories')->where('id', '=', $tabId)->where('status', '=', '1')->first();
			}
			if( $tabType == 'PRODUCT' ) {
				$data = DB::table('products')->where('id', '=', $tabId)->where('status', '=', '1')->first();
			}
...
function getMenuLink( $cms_link_id, $table_type, $table_id ) {
	...
	if( $cms_link_id != '' && $table_type != '' && $table_id != '' ) {
		...
		if( isset($cms) && !empty($cms) ) {
			...
			if( $tabType == 'DYNA_CONTENT' ) {
				$data = \App\Models\Content\Contents::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					$finalLink = route('front_cms_page', array('lng' => $lngCode, 'slug' => $data->slug));
				}
			}
			if( $tabType == 'PRODUCT_CATEGORY' ) {
				$data = \App\Models\Product\ProductCategories::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					$finalLink = route('front_cms_page', array('lng' => $lngCode, 'slug' => $data->slug));
				}
			...
			if( $tabType == 'PRODUCT' ) {
				$data = \App\Models\Product\Products::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					$finalLink = route('front_cms_page', array('lng' => $lngCode, 'slug' => $data->slug));
				}
			...
			if( $tabType == 'INDUSTRY' ) {
				$data = \App\Models\Industry\Industries::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					$finalLink = route('front_cms_page', array('lng' => $lngCode, 'slug' => $data->slug));
				}
			...
			if( $tabType == 'FLOWSHEET_CATEGORY' ) {
				$data = \App\Models\IndustryFlowsheet\FlowsheetCategories::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					$finalLink = route('front_cms_page', array('lng' => $lngCode, 'slug' => $data->slug));
				}
			...
			if( $tabType == 'FLOWSHEET' ) {
				$data = \App\Models\IndustryFlowsheet\Flowsheet::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					$finalLink = route('front_cms_page', array('lng' => $lngCode, 'slug' => $data->slug));
				}
			...
			if( $tabType == 'ARTICLE' ) {
				$data = \App\Models\Article\Articles::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					$finalLink = route('front.artCont', array('lng' => $lngCode, 'slug' => $data->slug));
				}
			}
			if( $tabType == 'EVENT' ) {
				$data = \App\Models\Events::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					$finalLink = route('front.evtCont', array('lng' => $lngCode, 'slug' => $data->slug));
				}
			}
			if( $tabType == 'PEOPLE_PROFILE' ) {
				$data = \App\Models\PeoplesProfile\PeoplesProfile::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					$finalLink = route('front.profCont', array('lng' => $lngCode, 'slug' => $data->slug));
				}
			}
			if( $tabType == 'DISTRIBUTOR_CATEGORY' ) {
				$data = \App\Models\Distributor\DistributorCategories::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					$finalLink = route('front.distrbCat', array('lng' => $lngCode, 'slug' => $data->slug));
				}
			}
			if( $tabType == 'DISTRIBUTOR' ) {
				$data = \App\Models\Distributor\Distributor::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
                    if( isset($data->distrCategorytOne) && isset($data->distrCategorytOne->catInfo) ) {
                        $catslug = $data->distrCategorytOne->catInfo->slug;
                        $finalLink = route('front.distrb', array('lng' => $lngCode, 'catslug' => $catslug, 'distrbslug' => $data->slug));
                    }
                }
			}
			if( $tabType == 'DISTRIBUTOR_CONTENT' ) {
				$data = \App\Models\Distributor\DistributorContents::where('id', '=', $tabId)->where('status', '=', '1')->first();
				if( !empty($data) ) {
					$lngCode = getLngCode($data->language_id);
					
                    if( isset($data->distributorInfo) && isset($data->distributorInfo->distrCategorytOne) && isset($data->distributorInfo->distrCategorytOne->catInfo) ) {
						$disslug = $data->distributorInfo->slug;
                        $catslug = $data->distributorInfo->distrCategorytOne->catInfo->slug;
                    	$finalLink = route('front.distrbCont', array('lng' => $lngCode, 'catslug' => $catslug, 'distrbslug' => $disslug, 'contslug' => $data->slug));
						// print_r($finalLink);die;
                    }
                }
			}
		}
...
function getReusableByKey($key) {

	$data = array();
	if( $key != '' && $key != null ) {
		$data = DB::table('reusable_content')->where('code_key', '=', $key)->first();
	}
...
function getLinkOrder($page_builder_id , $slug) {
...
function getProductImage( $product_id ) {
	...
	if( $product_id != '' ) {
		...
		->where('pim.image_type', '=', 'MAIN_IMAGE')
...
function getProductCatImage( $product_catid ) {

	$data = array();
	if( $product_catid != '' ) {
		$data = DB::table('product_categories_images_map as pim')
		->where('pim.product_category_id', '=', $product_catid)
		->where('pim.image_type', '=', 'MAIN_IMAGE')
		->join('image', 'image.id', '=', 'pim.image_id')
		->select('image.image', 'pim.*')
		->orderBy('pim.id', 'desc')
...
function getFileDownloadLinkFront( $file_id ) {

	$rtn = '';
	if( $file_id != '' && $file_id != '0' ) {
		$data = DB::table('files_search')->where('id', '=', $file_id)->first();
		if( !empty($data) ) {
			$rtn = $data->slug;
		}
	}
...
function getChildLngPageInfo( $table_type, $table_id, $lng_id ) {

	$data = array();
	if( $table_type != '' && $table_type != null && $table_id != '' && $table_id != null && $lng_id != '' && $lng_id != null) {

		$tabId = $table_id;
		$tabType = $table_type;

		if( $tabType == 'DYNA_CONTENT' ) {
			$data = DB::table('contents')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'PRODUCT_CATEGORY' ) {
			$data = DB::table('product_categories')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'PRODUCT' ) {
			$data = DB::table('products')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'ARTICLE' ) {
			$data = DB::table('articles')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'INDUSTRY' ) {
			$data = DB::table('industries')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'FLOWSHEET_CATEGORY' ) {
			$data = DB::table('flowsheet_category')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'FLOWSHEET' ) {
			$data = DB::table('flowsheet')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'DISTRIBUTOR' ) {
			$data = DB::table('distributor')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'DISTRIBUTOR_CATEGORY' ) {
			$data = DB::table('distributor_category')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'DISTRIBUTOR_CONTENT' ) {
			$data = DB::table('distributor_contents')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'PEOPLE_PROFILE' ) {
			$data = DB::table('peoples_profile')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
		if( $tabType == 'EVENT' ) {
			$data = DB::table('events')->where('parent_language_id', '=', $tabId)
			->where('status', '=', '1')->where('language_id', '=', $lng_id)->orderBy('id', 'desc')->first();
		}
	}
...
function productPageBanner($product_id) {
	$rtnArr = array();
	if($product_id != '') {
		$data = DB::table('products_images_map')->where('product_id', '=', $product_id)
		->where('image_type', '=', 'BANNER_IMAGE')->orderBy('id', 'desc')->first();
		if(!empty($data)) {
			$image = getImageById($data->image_id);
			if(!empty($image)) {
...
function getArticleThumbImage($atr_id) {
	$rtnArr = array();
	if($atr_id != '') {
		$data = DB::table('article_images_map')->where('article_id', '=', $atr_id)
		->where('image_type', '=', 'THUMB_IMAGE')->orderBy('id', 'desc')->first();
		if(!empty($data)) {
			$image = getImageById($data->image_id);
			if(!empty($image)) {
				$rtnArr['image'] = $image->image;
...
function getEventThumbImage($atr_id) {
	$rtnArr = array();
	if($atr_id != '') {
		$data = DB::table('event_images_map')->where('event_id', '=', $atr_id)
		->where('image_type', '=', 'THUMB_IMAGE')->orderBy('id', 'desc')->first();
...
function getHits($url=''){
	...
	// ->where('referral','like', '%'.$url.'%')
	// ->count();
	$hits=\App\Models\Referral::where('referral', 'like', '%'.$url.'%')->count();
