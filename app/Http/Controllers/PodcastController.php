...
class PodcastController extends Controller
{
 ...
 FROM podcast t1 where t1.status!=3 order by t1.id desc');
		...
		return view('dashboard.podcast.index', $DataBag);
	}
	...
	public function ajaxWebinars(Request $request)
	{
		...
		$DataBag['allProdCats'] = Podcast::with(['WebinarCategory'])
		...
		$returnHTML = view('dashboard.podcast.webinarFilter',$DataBag)->render();
	...
	public function allWbContent()
	{
		$DataBag = array();
		$DataBag['parentMenu'] = 'Podcast';
		$DataBag['childMenu'] = 'allPdContent';
		$DataBag['prodCat'] = PodcastContent::where('id', '=', '1')->first();

		return view('dashboard.podcast.webinarContent', $DataBag);
	}
	...
	public function updateWbContent(Request $request)
	{
		...
		if (isset($resx) && $resx == 1) {
			...
			return back()->with('msg', 'Podcast Content Updated Successfully.')
	...
	public function viewWbUser($id)
	{
		...
		$DataBag['allProdCats'] = PodcastUser::where('webinar_id', '=', $id)->where('status', '!=', 3)->orderBy('id', 'desc')->get();

		return view('dashboard.podcast.view', $DataBag);
	}
	...
	public function ajaxViewWbUser(Request $request,$id)
	{
		...
		// dd($DataBag);
		$returnHTML = view('dashboard.podcast.webinarviewFilter',$DataBag)->render();
		return response()->json(array('success' => true, 'html'=>$returnHTML));	
	...
	public function addWebinar()
	{
		...
		$DataBag['allSource'] = PodcastCategory::where('status', '=', '1')->select('name', 'id')
			->orderBy('name', 'asc')->get();

		$DataBag['alltopics'] = PodcastTopic::where('status', '=', '1')->select('name', 'id')
			->orderBy('name', 'asc')->get();

		$DataBag['allindustry'] = PodcastIndustry::where('status', '=', '1')->select('name', 'id')
			->orderBy('name', 'asc')->get();

		$DataBag['insert_id'] = md5(microtime(TRUE));
		return view('dashboard.podcast.add', $DataBag);
	}
	...
	public function saveWebinar(Request $request)
	{
		...
		$ck = Podcast::where('slug', '=', $slug_url)->where('id', '!=', $table_id)->count();
		if ($ck > 0) {
	...
	{
		$ck = PodcastUser::find($category_id);
		if (isset($ck) && !empty($ck)) {
			$ck->status = '3';
			$res = $ck->save();
			if (isset($res) && $res == 1) {
	...
	public function editWebinar($category_id, Request $request)
	{
		...
		$DataBag['prodCat'] = Podcast::where('status', '=', '1')->where('id', $category_id)->orderBy('name', 'asc')->first();
	...
	public function updateWebinar(Request $request, $category_id)
	{
		...
		if ($ck > 0) {
			...
			$Webinar = Podcast::find($category_id);
			$Webinar->name = trim(ucfirst($request->input('name')));
			...
			if (isset($resx) && $resx == 1) {
				...
				return redirect()->route('allPd')->with('msg', 'Webinar Updated Successfully.')
					->with('msg_class', 'alert alert-success');
			}
	...
	public function downloadWebinarUsers(Request $request)
	{
	 ...
	 public function allPodcastContent(Request $request){
        $DataBag = array();
		$DataBag['parentMenu'] = 'podcast';
		$DataBag['childMenu'] = 'podcast';
		$DataBag['prodCat'] = PodcastContent::where('id', '=', '1')->first();

		return view('dashboard.podcast.brochurecontent', $DataBag);
    }

    public function updatePodcastContent(Request $request)
	{
