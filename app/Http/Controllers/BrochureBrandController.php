...
class BrochureBrandController extends Controller
{
    public function allBrochureIndustry() {
        $DataBag = array();
     $DataBag['parentMenu'] = 'Brochure';
     $DataBag['childMenu'] = 'brochurebrand';
     $DataBag['allProdCats'] = DB::table('brochure_brand')->where('status', '!=', '3')->orderBy('id', 'desc')->get();
     

     return view('dashboard.brochurebrand.index', $DataBag);
    } 
 ...
 public function addBrochureIndustry() {
     ...
     return view('dashboard.brochurebrand.add', $DataBag);
 }
 ...
 public function saveBrochureIndustry(Request $request) {
     ...
     $WebinarIndustry = new BrochureBrand;
     $WebinarIndustry->name = trim( ucfirst($request->input('name')) );
          
     $resx = $WebinarIndustry->save();
     if( isset($resx) && $resx == 1 ) {

        return redirect()->route('allBrbrandId')->with('msg', 'Brochure  Brand Updated Successfully.')
 ...
 public function deleteBrochureIndustry($topic_id) {
     $ck = BrochureBrand::find($topic_id);
     if( isset($ck) && !empty($ck) ) {
         $ck->status = '3';
         $res = $ck->save();
         if( isset($res) && $res == 1 ) {

             return back()->with('msg', 'Brochure Brand Deleted Successfully.')
             ->with('msg_class', 'alert alert-success');
         }
     }
 ...
 public function editBrochureIndustry($topic_id,Request $request) {
     ...
     $DataBag['prodCat'] = BrochureBrand::where('status', '=', '1')->where('id',$topic_id)->orderBy('name', 'asc')->first();
 ...
 public function updateBrochureIndustry(Request $request, $topic_id) {
         ...
         $WebinarIndustry = BrochureBrand::find($topic_id);
         $WebinarIndustry->name = trim( ucfirst($request->input('name')) );
         ...
         if( isset($resx) && $resx == 1 ) {
             ...
             return redirect()->route('allBrbrandId')->with('msg', 'Brochure Type Updated Successfully.')
             ->with('msg_class', 'alert alert-success');
