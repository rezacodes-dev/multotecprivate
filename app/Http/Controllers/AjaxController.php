...
class AjaxController extends Controller
{
    ...
    public function ajaxPodcast(Request $request){
        $lng='en';
 
        $DataBag = array();  
         
        $query = \App\Models\Podcast::where('status', '=', '1')->with('WebinarCategory');
        ...
        if(isset($listData)  && count($listData)>0){
            ...
            $gh=route('front.podcastContent', array('lng' => $lng, 'id' => $v->slug));
            ...
            <ul>
            <li><a >Product: '.$v->webinarcat.'</a></li> 
            <li><a href="'.$gh.'">Watch Podcast</a></li>  
            </ul>
            <p>'.html_entity_decode( $v->short_description ).'</p>
            </div>
            </div>
