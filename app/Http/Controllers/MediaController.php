...
/*********************************************** FILES ***************************************************/
    ...
    public function upload_file(Request $request) {
        ...
        if( $request->hasFile('main_file') ) {
           ...
           $pdf = $pdfParser->parseFile($main_file->path());
           $content = $pdf->getText();
        ...
        if( $FilesMaster->save() ) {
            ...
            $FileCategoriesMap->save();
              ...
              DB::table('files_search')->insert([
                'name'=>$content??'',
                'file_master_id'=>$file_id??'',
                'description'=>trim($request->input('title'))??'',
                'slug'=>$file_newname??'',
                'status'=>1,
            ]);
    ...
    public function update_file(Request $request, $file_id) {
      // dd('Hello');
        ...
        if( $FilesMaster->save() ) {
            if( $request->hasFile('main_file') ) {
            DB::table('files_search')->where('file_master_id',$file_id)->update([
                'name'=>$content,
                'description'=>trim($request->input('title')),
                'slug'=>$file_newname,
                'status'=>1,
    ...
    public function filecontents()
    {
        ...
        try {
            $items = DB::table('files_master')
                 ->select('id', 'title', 'file')
                  ->where('id','=',239)
                //   ->where('id','>',359)
                //   ->where('id','<',380)
                //  ->where('id','=',182)
              //   ->whereNotIn('id',[332,333,334,335,356,357,358])  
                //  ->whereNotIn('id',[181,183])  
                ->get();
    
            foreach ($items as $item) {
                // Process each item here
                $pdfParser = new \Smalot\PdfParser\Parser();
                $path = 'public/uploads/files/media_files/' . $item->file??'';
                $filePath = url($path);
          ...
          //    dd(html_entity_decode($content));
            ...
            echo "Query exception occurred: " . $e->getMessage();
        }
    }
