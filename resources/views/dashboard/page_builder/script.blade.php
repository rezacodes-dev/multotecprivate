					...
					if( respx != '' ) {
						...
						if( _builderType == 'PRODUCT_LINKS' || _builderType == 'DISTRIBUTOR' || _builderType == 'DISTRIBUTOR_PAGE' || _builderType == 'PRODUCT_CAT_LINKS' || _builderType == 'PEOPLE_LINKS' || _builderType.includes('CONTENT_LINKS') || _builderType == 'NEWS_LINKS' || _builderType == 'BROCHURE_LINKS' ) {
							...
							for( var c = 0; c < linksObjLen; c++ ) {
								...
								viewHtml += '<tr style="background-color:'+ trcolor +'">';
									viewHtml += '<td>';
										if( linksObj[ c ].isSelected == 'YES') {
											viewHtml += '<input type="checkbox" name="slugs[]" checked="checked" class="ckblinks" data="' + linksObj[ c ].id + '" value="' + linksObj[ c ].slug +'">&nbsp;';
										} else {
											viewHtml += '<input type="checkbox" name="slugs[]" class="ckblinks" data="' + linksObj[ c ].id + '" value="' + linksObj[ c ].slug +'">&nbsp;';
										}
										viewHtml += linksObj[ c ].name ;
									viewHtml += '</td>';
									viewHtml += '<td>';
										if( linksObj[ c ].isSelected == 'YES') {
											viewHtml += '<input type="text" name="link_order[]" class="onlyNumber" id="ordbox_'+ linksObj[ c ].id +'" style="width: 35px;" value="'+ linksObj[ c ].order +'">';
										} else {
											viewHtml += '<input type="text" name="link_order[]" class="onlyNumber" id="ordbox_'+ linksObj[ c ].id +'" style="width: 35px;" disabled="disabled" value="'+ linksObj[ c ].order +'">';
										}
									viewHtml += '</td>';
									viewHtml += '<td>';
							...
							$('#linkboxTAB tbody').html( viewHtml );
						...
						if( _builderType == 'PRODUCT_BOX' || _builderType == 'PRODUCT_CAT_BOX' ) {
							
							$('#PrdBoxLINKS_this_id').val( _ID );
							$('#prdbox_link_heading').val( _mainTitle );
							$('#PrdBoxlinks_builder_type').val( _builderType );
							$('#boxlinkType').attr('disabled','disabled');
							...
							$('#prodbox_linkboxTAB tbody').html( viewHtml );
...
$(document).ready(function(){
  $("body").on("keyup", ".xSRC", function() {
    var value = $(this).val().toLowerCase();
    $(".srcTab tr").filter(function() {
      $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1);
    });
  });
  $('body').on('change', '.ckblinks', function() {
    if( $(this).is(':checked') ) {
      $('#ordbox_' + $(this).attr('data')).removeAttr('disabled');
    } else {
      $('#ordbox_' + $(this).attr('data')).val('0').attr('disabled', 'disabled');
    }
    var ckblnks = $('.ckblinks').filter(':checked').length;
    if( ckblnks > 0 ) {
      $('#addPrdBoxSeleLinks').removeAttr('disabled');
    } else {
