                    ...
                    <section class="green_strip">
                        <div class="container">
                            <div class="text-center">
                                <h6>{!! trim( html_entity_decode( $pgd->main_title, ENT_QUOTES ) ) !!}{{-- $pgd->main_title --}}<a href="{{$pgd->link_url}}">{{ $pgd->link_text }}</a></h6>
                            </div>
                        </div>
                    </section>
                @endif
                                                ...
                                                <div class="item">
                                                    <div class="innerslide">
                                                        <div class="video_block" style="width: 75%;">
                                                            <iframe width="560" height="315" src="https://www.youtube.com/embed/{{ $vidGal->masterVideoInfo->video_link }}?rel=0" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>   
                                                        </div>
...
@if( isset($allData->pageBuilderContent) && !empty($allData->pageBuilderContent) && isset($device) )
    ...
    @if( $pgd->device == $device || $pgd->device == '3' ) <!-- Device Checking -->
        ...
        @if( $pgd->builder_type == 'HERO_SPW' && $pgd->position == 'BODY' )
            <div class="padtop"><div class="container">{!! trim( html_entity_decode( $pgd->main_content, ENT_QUOTES ) ) !!}</div></div>
            <div class="clearfix"></div> 
        @endif
